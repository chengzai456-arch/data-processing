  ...
  try {
    ...
    if (ct.includes('application/json')) {
      ...
      function jsonToBuffer(rows: any[]): Buffer {
        ...
        const ws = XLSX.utils.aoa_to_sheet([h, ...rows.map((r: any) => h.map((k: string) => r[k]))]);
      ...
      for (const key of ['roster','leave','makeup','sign_this','sign_last','sign_biweek','gus_whitelist','shift_dict']) {
      ...
      const result = await runPipeline(files, workerType, supabase, uploadId, targetDate);
    ...
    for (const [key, val] of formData.entries()) {
      if (val instanceof File) files[key] = Buffer.from(await val.arrayBuffer());
      else fields[key] = String(val);
    }
    if (!files['file']) return NextResponse.json({ error: '缺少原始数据' }, { status: 400 });
    ...
    const uploadId = crypto.randomUUID();
    const result = await runPipeline(files as any, fields['worker_type'] || 'GUS', supabase, uploadId, fields['target_date'] || '');
    return NextResponse.json(result);
